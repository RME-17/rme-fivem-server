...
function renderBasic(){
  ...
  const propOpts=PROPS.map(p=>'<option value=\"'+esc(p)+'\"'+(editing.prop===p?' selected':'')+'>'+esc(p)+'</option>').join('');
  ...
  pane.querySelector('#f-label').oninput=(e)=>editing.label=e.target.value;
  pane.querySelector('#f-prop').onchange=(e)=>editing.prop=e.target.value;
}

function renderAccess(){
  const pane=document.querySelector('[data-pane=\"access\"]');
  const jobOpts=JOBS.map(j=>'<option value=\"'+esc(j.name)+'\"'+(editing.accessValue===j.name?' selected':'')+'>'+esc(j.label)+'</option>').join('');
  const gangOpts=GANGS.map(g=>'<option value=\"'+esc(g.name)+'\"'+(editing.accessValue===g.name?' selected':'')+'>'+esc(g.label)+'</option>').join('');
  const modes=['public','job','gang'].map(a=>'<label class=\"radio '+(editing.access===a?'sel':'')+'\"><input type=\"radio\" name=\"acc\" value=\"'+a+'\"'+(editing.access===a?' checked':'')+'/> '+(a.charAt(0).toUpperCase()+a.slice(1))+'</label>').join('');
  ...
  pane.querySelectorAll('input[name=\"acc\"]').forEach(r=>r.onchange=(e)=>{
  ...
  const jb=pane.querySelector('#f-job'); if(jb) jb.onchange=(e)=>editing.accessValue=e.target.value;
  const gr=pane.querySelector('#f-grade'); if(gr) gr.oninput=(e)=>editing.accessGrade=parseInt(e.target.value)||0;
  const gg=pane.querySelector('#f-gang'); if(gg) gg.onchange=(e)=>editing.accessValue=e.target.value;
}
...
function renderRecipes(){
  ...
  add.onclick=()=>{ editing.recipes.push({ output:'', amount:1, time:5000, materials:[] }); renderRecipes(); };
  pane.appendChild(add);
  editing.recipes.forEach((r,ri)=>{
    ...
    out.querySelector('.ro-pick').onclick=()=>openPicker(name=>{ r.output=name; renderRecipes(); });
    ...
    row.querySelector('.r-amt').oninput=(e)=>r.amount=parseInt(e.target.value)||1;
    row.querySelector('.r-time').oninput=(e)=>r.time=parseInt(e.target.value)||5000;
    card.appendChild(row);
    const mats=document.createElement('div'); mats.className='mats'; mats.innerHTML='<div class=\"mats-label\">Materials</div>';
    (r.materials||[]).forEach((m,mi)=>{
      ...
      mrow.querySelector('.mat-pick').onclick=()=>openPicker(name=>{ m.item=name; renderRecipes(); });
      mrow.querySelector('.m-amt').oninput=(e)=>m.amount=parseInt(e.target.value)||1;
      mrow.querySelector('button').onclick=()=>{ r.materials.splice(mi,1); renderRecipes(); };
    ...
    addMat.onclick=()=>{ r.materials=r.materials||[]; r.materials.push({ item:'', amount:1 }); renderRecipes(); };
    ...
    del.onclick=()=>{ editing.recipes.splice(ri,1); renderRecipes(); };
...
function renderPickerCats(){
  ...
  cats.forEach(c=>{ const d=document.createElement('div'); d.className='pcat'+(c===PICKER_CAT?' active':''); d.textContent=c; d.onclick=()=>{ PICKER_CAT=c; renderPickerCats(); renderPickerGrid(); }; el.appendChild(d); });
}
function renderPickerGrid(){
  ...
  for(const it of ITEMS){
    ...
    d.onclick=()=>{ if(pickerTarget) pickerTarget(it.name); document.getElementById('itemPicker').classList.add('hidden'); };
    el.appendChild(d);
...
function openCraft(d){
  ITEMS=d.items||[]; ITEM_MAP={}; ITEMS.forEach(i=>ITEM_MAP[i.name]=i);
  applyTheme(d.theme||{});
  const b=d.bench||{};
  ...
  (b.recipes||[]).forEach((r,i)=>{
    const outItem=ITEM_MAP[r.output];
    const mats=(r.materials||[]).map(m=>{ const mi=ITEM_MAP[m.item]; return '<div class=\"cm\"><img src=\"'+(mi?mi.image:'')+'\" onerror=\"this.style.visibility=\'hidden\'\"/><span>'+(m.amount||1)+'x '+esc(mi?mi.label:m.item)+'</span></div>'; }).join('');
    ...
    card.querySelector('button').onclick=()=>{ post('craft', { index:i+1 }); };
...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  document.getElementById('closeCreator').onclick=()=>{ closeAll(); post('close'); };
  document.getElementById('saveBench').onclick=()=>{ if(editing){ post('saveBench', editing); closeAll(); } };
  document.getElementById('placeBench').onclick=()=>{ if(editing){ closeAll(); post('placeBench', editing); } };
  document.getElementById('deleteBench').onclick=()=>{ if(editing && editing.id){ post('deleteBench', { id:editing.id }); closeAll(); } };
  document.querySelectorAll('.tab').forEach(t=>t.onclick=()=>switchTab(t.dataset.tab));
  document.getElementById('pickerClose').onclick=()=>document.getElementById('itemPicker').classList.add('hidden');
  document.getElementById('pickerSearch').oninput=renderPickerGrid;
  document.getElementById('closeCraft').onclick=()=>{ closeAll(); post('close'); };
});
