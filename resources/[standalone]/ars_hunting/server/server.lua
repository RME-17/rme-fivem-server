...
RegisterNetEvent("ars_hunting:finishMission", function(data)
    ...
    -- RME: QBCore has no 'money' item, so pay cash rewards via addMoney instead
    for _, reward in pairs(data.rewards) do
        if reward.item == "money" then
            framework.addMoney({ target = source, amount = reward.quantity })
        else
            framework.addItems({ target = source, items = { reward } })
        end
    end
