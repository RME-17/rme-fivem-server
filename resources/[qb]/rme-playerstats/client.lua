...
-- combat) and turns it into levelled skills. Press END to open/close the panel.
-- Stats are loaded from / saved to the database per character (citizenid).
--
...
local function fetchStats()
    QBCore.Functions.TriggerCallback('rme-playerstats:server:get', function(data)
        if data and type(data) == 'table' then Stats = data end
    end)
end
...
local function buildPayload()
    ...
    local skills = {}
    local function add(label, icon, xp, sub)
        local lvl, pct = levelInfo(xp)
        skills[#skills + 1] = { label = label, icon = icon, level = lvl, pct = pct, sub = sub }
    end

    add('Running', '\240\159\143\131',
        (s.sprint_distance or 0) * Config.Xp.sprintPerMeter + (s.run_distance or 0) * Config.Xp.joggPerMeter,
        fmtDist(s.sprint_distance) .. ' sprinted')
    add('Swimming', '\240\159\143\138',
        (s.swim_distance or 0) * Config.Xp.swimPerMeter,
        fmtDist(s.swim_distance) .. ' swum')
    add('Shooting', '\240\159\142\175',
        (s.shots_hit or 0) * Config.Xp.hit,
        comma(s.shots_hit or 0) .. ' shots on target')
    add('Driving', '\240\159\154\151',
        (s.drive_distance or 0) * Config.Xp.drivePerMeter,
        fmtDist(s.drive_distance) .. ' driven')
    add('Flying', '\226\156\136\239\184\143',
        (s.fly_distance or 0) * Config.Xp.flyPerMeter,
        fmtDist(s.fly_distance) .. ' flown')
    add('Stamina', '\240\159\171\129',
        (s.sprint_distance or 0) * 0.05 + (s.swim_distance or 0) * 0.08,
        fmtTime(s.playtime) .. ' active')
    add('Strength', '\240\159\146\170',
        (s.kills or 0) * Config.Xp.kill,
        comma(s.kills or 0) .. ' takedowns')
    ...
    local overview = {
        ...
        { label = 'Shots fired',      value = comma(s.shots_fired or 0) },
        { label = 'Shots hit',        value = comma(s.shots_hit or 0) },
        { label = 'Accuracy',         value = acc .. '%' },
        { label = 'Kills',            value = comma(s.kills or 0) },
